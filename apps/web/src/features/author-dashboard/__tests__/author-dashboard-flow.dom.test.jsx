...
vi.mock("../api", async () => {
  ...
  return {
    ...actual,
    bootstrapAuthorDashboardSession: (...args) =>
      bootstrapAuthorDashboardSessionMock(...args),
  };
});

const fetchMyAuthorApplicationMock = vi.fn();
const fetchAdminAuthorApplicationsMock = vi.fn();
vi.mock("../../author-applications/api", () => ({
  fetchMyAuthorApplication: (...args) => fetchMyAuthorApplicationMock(...args),
  fetchAdminAuthorApplications: (...args) =>
    fetchAdminAuthorApplicationsMock(...args),
  approveAuthorApplication: vi.fn(),
  rejectAuthorApplication: vi.fn(),
}));
...
describe("Author dashboard route guard", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    fetchMyAuthorApplicationMock.mockResolvedValue({
      ok: true,
      data: { application: null },
    });
    fetchAdminAuthorApplicationsMock.mockResolvedValue({
      ok: true,
      data: {
        items: [],
        summary: {
