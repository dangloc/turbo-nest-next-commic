...
describe("local auth forms", () => {
  ...
  it("submits valid login and redirects on success", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("So dien thoai, ten dang nhap, email"), {
      target: { value: "user@example.com" },
    });
    fireEvent.change(screen.getByPlaceholderText("Mat khau"), {
  ...
  it("shows duplicate identity error for registration failure", async () => {
    ...
    fireEvent.change(screen.getAllByPlaceholderText(/ten/i)[0], {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Dang ky" }));
