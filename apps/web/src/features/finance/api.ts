...
export async function fetchVipPackages(
  token?: string,
  signal?: AbortSignal,
): Promise<ApiResult<VipPackagesResponse>> {
  return apiRequest<VipPackagesResponse>("/finance/purchases/vip/packages", {
    method: "GET",
    headers: authHeaders(token),
    includeCredentials: true,
    signal,
...
export async function purchaseVipPackage(
  packageType: VipPackageType,
  token?: string,
): Promise<ApiResult<PurchaseVipPackageResponse>> {
  return apiRequest<PurchaseVipPackageResponse>(
    `/finance/purchases/vip/packages/${packageType}`,
    {
      method: "POST",
      headers: authHeaders(token),
      includeCredentials: true,
...
export async function purchaseChapter(
  ...
  if (!result.ok) {
    if (
      ...
      /insufficient deposited balance(?: or reward points)?/i.test(result.error.message)
    ) {
      return {
        ok: true,
        data: {
  ...
  return {
    ...
    data: {
      ...
      pointsSpent: result.data.pointsSpent,
