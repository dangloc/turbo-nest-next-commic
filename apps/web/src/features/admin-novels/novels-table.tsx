...
export function AdminNovelsTable() {
  ...
  const isAuthorUser = user?.role === "AUTHOR" && user?.id !== 1;
  ...
  async function fetchNovels(nextQuery: NovelListQuery) {
    ...
    const res = await listNovels(
      toRequestQuery({
        ...nextQuery,
        scope: isAuthorUser ? "mine" : nextQuery.scope,
      }),
      undefined,
      controller.signal,
    );
  ...
  useEffect(() => {
    if (!isAuthorUser || query.scope === "mine") {
      return;
    }

    setQuery((prev) => ({ ...prev, scope: "mine", page: 1 }));
  }, [isAuthorUser, query.scope]);
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              ...
              <th className="px-4 py-3 text-left font-medium text-muted-foreground hidden md:table-cell w-16">
                Ảnh
              </th>
              <th className="px-4 py-3 text-left font-medium text-muted-foreground">
                Tiêu đề
