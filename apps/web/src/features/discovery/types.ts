export type DiscoverySortBy = "viewCount" | "updatedAt" | "createdAt" | "recommendationVotes";
...
export function normalizeDiscoveryQuery(init: DiscoveryQueryInit = {}): DiscoveryQuery {
  ...
  const sortBy =
    init.sortBy === "viewCount" ||
    init.sortBy === "createdAt" ||
    init.sortBy === "updatedAt" ||
    init.sortBy === "recommendationVotes"
      ? init.sortBy
      : DISCOVERY_DEFAULTS.sortBy;
  const sortDir = init.sortDir === "asc" || init.sortDir === "desc" ? init.sortDir : DISCOVERY_DEFAULTS.sortDir;

  return {
    q: normalizeText(init.q),
    author: normalizeText(init.author),
    releaseYear: normalizeText(init.releaseYear),
    page,
    limit,
    sortBy,
    sortDir,
...
export function parseDiscoverySearchParams(searchParams: { get(name: string): string | null }) {
  return normalizeDiscoveryQuery({
    ...
    releaseYear: searchParams.get("releaseYear") ?? searchParams.get("year"),
...
export function mergeDiscoveryQuery(
  ...
  return normalizeDiscoveryQuery({
    ...
    releaseYear: overrides.releaseYear ?? base.releaseYear,
