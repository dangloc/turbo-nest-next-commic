...
import { getDashboardLandingHref } from "../lib/dashboard-access";
...
export function Header() {
  ...
  function onSearch(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(trimmed ? `/novels?q=${encodeURIComponent(trimmed)}` : "/novels");
  }
  ...
  return (
    <header className="tsh2-header">
      <div className="tsh2-header__row tsh2-header__row--top">
        ...
        <form className="tsh2-search" role="search" onSubmit={onSearch}>
          <Input
            className="tsh2-search__input"
            type="search"
        ...
        <div className="tsh2-topActions">
          <Link href="/vip" className="tsh2-linkBtn tsh2-vipBtn">{copy.vip}</Link>
      ...
      <div className="tsh2-header__row tsh2-header__row--bottom">
        <div className="tsh2-leftCluster">
          ...
          <nav className="tsh2-nav" aria-label="Main">
            ...
            <Link href="/novels">{copy.novels}</Link>
            <Link href="/bookmarks">{copy.bookmark}</Link>
          </nav>
        </div>

        <div className="tsh2-rightCluster">
          <Link href="/top-up" className="tsh2-linkBtn tsh2-topupBtn">{copy.topup}</Link>
          <Select
            value={locale}
          ...
          <div className="tsh2-account" ref={menuRef}>
            ...
            >
              <Avatar fallback={initials} src={avatarUrl} alt={user?.displayName ?? user?.email ?? "User"} />
            </button>
            ...
            {menuOpen ? (
              <div className="tsh2-account__menu" role="menu">
                ...
                <Link href="/profile/transactions" onClick={() => setMenuOpen(false)}>
                  {copy.wallet}
                </Link>
